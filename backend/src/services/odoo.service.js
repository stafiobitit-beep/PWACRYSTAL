...
export class OdooTaskService {
    static async getTasks(domain = [], since) {
        if (since) {
...
export class OdooPartnerService {
    static async getPartners(domain = [['customer_rank', '>', 0]]) {
...
export class OdooMessageService {
    static async postMessage(taskId, content, senderName) {
...
export class OdooTimesheetService {
    static async createTimesheet(taskId, minutes, description, cleanerEmail) {
        ...
        if (cleanerEmail) {
            try {
                const users = await odoo.execute('res.users', 'search_read', [[['login', '=', cleanerEmail], ['active', '=', true]]], { fields: ['id', 'employee_ids'] });
                if (users?.[0]) {
...
export class OdooAttachmentService {
    static async uploadAttachment(taskId, fileName, base64Data) {
...
export class OdooUserService {
    static async findByEmail(email) {
        try {
            const users = await odoo.execute('res.users', 'search_read', [[['login', '=', email]]], { fields: ['id', 'name', 'email', 'employee_ids'] });
            return users?.[0] || null;
        }
        catch {
            return null;
    ...
    static async createInternalUser(name, email) {
        try {
