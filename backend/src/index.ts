...
app.delete('/api/users/:id', authenticateToken, authorizeRole(['ADMIN']), async (req, res) => {
  try {
    ...
    if (activeTasks > 0) return res.status(409).json({ message: 'Gebruiker heeft actieve taken' });
...
app.get('/api/odoo/users', authenticateToken, authorizeRole(['ADMIN']), async (_req, res) => {
  try {
    const odooUsers = await odoo.execute(
      'res.users',
      'search_read',
    ...
    const filtered = (odooUsers as any[]).filter((u: any) => u.id > 3);
    res.json(filtered);
  } catch (e: any) {
...
app.delete('/api/customers/:id', authenticateToken, authorizeRole(['ADMIN']), async (req, res) => {
  ...
  await prisma.location.updateMany({ where: { customerId: id }, data: { customerId: undefined as any } });
  await prisma.user.delete({ where: { id } });
